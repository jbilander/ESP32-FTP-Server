#ifndef _RNFR_RNTO_H_
#define _RNFR_RNTO_H_
...
{
public:
    explicit RNFR_RNTO(WiFiClient *const Client, SdFs *const Filesystem) : FTPCommand("RN", 1, Client, Filesystem), _fromSet(false) {}

    void run(FTPPath &WorkDirectory, const std::vector<String> &Line) override
    {
        if (Line[0].equals("RNFR"))
        {
            from(WorkDirectory, Line);
        }
        else if (Line[0].equals("RNTO"))
        {
