...
class MFMT : public FTPCommand
{
public:
    explicit MFMT(WiFiClient *const Client, SdFs *const Filesystem) : FTPCommand("MFMT", 1, Client, Filesystem) {}
    ...
    void run(FTPPath &WorkDirectory, const std::vector<String> &Line) override
    {
        ...
        if (f.isFile() || f.isDir())
        {
            ...
            f.timestamp(T_WRITE, YYYY, MM, DD, HH, MI, SS);
            f.close();
